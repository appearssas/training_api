...
export class ReportsService {
  ...
  async getStats(filters: ReportFilters, user?: Usuario) {
    ...
    const topCourses = Array.from(courseMap.values())
      ...
      .map(c => ({
        ...
        completionRate: Math.round(
          (c.count > 0 ? c.passed / c.count : 0) * 100,
        ), // Approximate completion as passed/total
    ...
    const completionTrend = Array.from(trendMap.entries()).map(
    ...
    const certificatesTrend = Array.from(certTrendMap.entries()).map(
      ([label, value]) => ({ label, value }),
    );
