...
export class CargaMasivaConductoresUseCase {
  ...
  async execute(fileBuffer: Buffer): Promise<CargaMasivaResponseDto> {
    ...
    try {
      ...
      throw new BadRequestException(
        `Error al parsear el archivo CSV: ${error.message}`,
      );
    }
    ...
    for (let i = 0; i < filas.length; i++) {
      ...
      try {
        ...
        this.validarFila(fila, numeroFila);
        ...
        this.logger.error(
          `Error en fila ${numeroFila}: ${error.message}`,
          error.stack,
        );
        errores.push({
          fila: numeroFila,
  ...
  private validarFila(fila: any, numeroFila: number): void {
    // Validar campos obligatorios
    if (!fila.numeroDocumento || !fila.numeroDocumento.trim()) {
      throw new Error('El número de documento es obligatorio');
    }

    if (!fila.nombres || !fila.nombres.trim()) {
      throw new Error('Los nombres son obligatorios');
    }

    if (!fila.apellidos || !fila.apellidos.trim()) {
    ...
    if (fila.email && fila.email.trim()) {
      ...
      if (!emailRegex.test(fila.email.trim())) {
