    ...
    for (const inscripcion of inscripcionesAprobadas) {
      ...
      const cert =
        inscripcion.certificados && inscripcion.certificados.length > 0
          ? inscripcion.certificados[0]
          : null;

      if (cert) {
        this.logger.log(
          `Updating dynamic URL for Inscripcion ${inscripcion.id}`,
        );
        try {
          // Asegurar que existe hash
          const hash = cert.hashVerificacion;
          if (!hash) {
            ...
            this.logger.warn(
              `Certificado ${cert.id} sin hash. Saltando a recreación completa.`,
            );
          ...
          if (hash) {
            ...
            this.logger.log(
              `URL dinámica actualizada para certificado ID: ${cert.id}`,
            );
            result.generados++;
          }
        } catch (error) {
          this.logger.error(
            `Error actualizando certificado ${inscripcion.id}`,
            error,
          );
          result.errores++;
          if (error instanceof Error)
            result.detallesErrores.push(
              `Update Inscripcion ${inscripcion.id}: ${error.message}`,
            );
        }
        ...
        try {
          ...
          result.generados++;
          this.logger.log(
            `Certificado (metadata) generado para inscripción ID: ${inscripcion.id}`,
          );
        } catch (error) {
          result.errores++;
          const errMsg =
          ...
          result.detallesErrores.push(
            `Inscripcion ${inscripcion.id}: ${errMsg}`,
          );
          this.logger.error(
            `Error generando certificado para inscripción ${inscripcion.id}`,
            error,
          );
        }
      }
